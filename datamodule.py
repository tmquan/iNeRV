    ...
    def __init__(self,
        ...
        def glob_files(folders: str = None, extension: str = '*.nii.gz'):
            ...
            paths = [glob.glob(os.path.join(folder, extension),
                               recursive=True) for folder in folders]
    ...
    def train_dataloader(self):
        self.train_transforms = Compose(
            [
                ...
                Spacingd(keys=["image3d"], pixdim=(1.0, 1.0, 1.0),
                ...
                CropForegroundd(keys=["image3d"], source_key="image3d", select_fn=(
                    lambda x: x > 0), margin=0),
                CropForegroundd(keys=["image2d"], source_key="image2d", select_fn=(
                    lambda x: x > 0), margin=0),
                ...
                RandZoomd(keys=["image3d"], prob=1.0, min_zoom=0.8, max_zoom=1.1,
                          padding_mode='constant', mode=["trilinear"], align_corners=True),
                RandZoomd(keys=["image2d"], prob=1.0, min_zoom=0.8,
                          max_zoom=1.1, padding_mode='constant', mode=["area"]),
                Resized(keys=["image3d"], spatial_size=self.vol_shape, size_mode="longest", mode=[
                        "trilinear"], align_corners=True),
                Resized(keys=["image2d"], spatial_size=self.img_shape,
                        size_mode="longest", mode=["area"]),
                DivisiblePadd(keys=["image3d"], k=self.vol_shape,
                              mode="constant", constant_values=0),
                DivisiblePadd(keys=["image2d"], k=self.img_shape,
                              mode="constant", constant_values=0),
